...
exponential_serve_rate = False
average_system_time = []

for lambda_ in np.arange(0.01,2.01,0.01):
    ...
    average_system_time.append(np.mean(system_time[n:n+1000]))

print("The average sojourn time is:",np.mean(average_system_time))

#plot the figure of sojurn time
plt.figure()
plt.plot(np.arange(0.01,2.01,0.01),average_system_time)
plt.xlabel('Arrivate rate lambda')
plt.ylabel('Average sojourn time')
plt.savefig(f'./average sojourn time under different arrival rate',dpi=500)
